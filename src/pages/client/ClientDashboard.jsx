        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="bg-gray-800 p-4 rounded-xl border border-gray-700 flex flex-col space-y-2">
              <button 
                onClick={() => setActiveTab('my_tenders')}
                className={`w-full text-left px-4 py-3 rounded font-medium transition ${activeTab === 'my_tenders' ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:bg-gray-700 hover:text-white'}`}
              >
                My Tenders
              </button>
              <button 
                onClick={() => setActiveTab('create')}
                className={`w-full text-left px-4 py-3 rounded font-medium transition ${activeTab === 'create' ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:bg-gray-700 hover:text-white'}`}
              >
                Create New Tender
              </button>
            </div>
          ...
          <div className="lg:col-span-3 bg-gray-800 p-6 rounded-xl border border-gray-700 min-h-[500px]">
            {activeTab === 'create' && <CreateTender onSuccess={() => setActiveTab('my_tenders')} />}
            {activeTab === 'my_tenders' && <MyTenders />}
          </div>
        </div>
      </div>
    </div>
